  ...
  const handleSubmit = async () => {
    if (validateInputs()) {
      try {
        ...
        const url =
          "https://stylesync-backend-test.onrender.com/app/v1/salon/salon-login";
    ...
    <ImageBackground source={BACKGROUND_IMAGE} style={globalStyles.background}>
      ...
      >
        <View style={[globalStyles.container]}>
          <View>
            <Text style={globalStyles.mainTopic}>{"Login\nWelcome back!"}</Text>
          </View>
          <View style={{ marginTop: 35 }}>
            {validation ? (
              <Text style={LoginStyles.ErrorText}>{validation}</Text>
            ) : null}
            <View
              style={[
                LoginStyles.PasswordInputArea,
          ...
          <View
            style={[
              ...
              errorPassword ? { borderColor: "red" } : null,
            ]}
          >
            <View style={{ flex: 1 }}>
              <TextInput
...
function BottomFeild({ navigation, onPress }) {
  return (
    <View>
      <View>
        <TouchableOpacity>
          <Text style={LoginStyles.FogotP}>{"Forgot Password?"}</Text>
        </TouchableOpacity>
      ...
      <View style={LoginStyles.BottomSection}>
        ...
        >
          <Text style={LoginStyles.BottomSignUp}>{"Signup"}</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
